    ...
    return (
        <div className="p-6 space-y-6">
            <header>
                ...
                <p className="text-gray-500 text-sm">All bookings — completed, rejected, emergency ended, and more</p>
            </header>
                ...
                <Card>
                    <CardContent className="p-12">
                        <div className="text-center space-y-3">
                            ...
                            <h3 className="text-lg font-semibold text-gray-900">Select Sport & Date</h3>
                ...
                <>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {STATUS_COUNTS.filter(s => statusCounts[s] > 0).map(s => (
                            <span key={s} className={`px-3 py-1 rounded-full text-xs font-bold capitalize ${STATUS_COLORS[s]}`}>
                                {s}: {statusCounts[s]}
                            </span>
                        ))}
                        {logs.length === 0 && (
                        ...
                        <div className="bg-white rounded-xl border overflow-hidden shadow-sm">
                            <div className="px-5 py-4 border-b flex items-center gap-2">
                                ...
                                <span className="font-semibold text-gray-900 text-sm">
                                    {format(new Date(date), 'EEEE, MMMM d yyyy')} · {sport.charAt(0).toUpperCase() + sport.slice(1)}
                                </span>
                                <span className="ml-auto text-xs text-gray-400">{logs.length} bookings</span>
                            </div>
                                ...
                                <table className="w-full text-sm">
                                    <thead className="bg-gray-50 border-b">
                                        <tr>
                                            ...
                                            <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-gray-500">Court</th>
                                            <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-gray-500">Booked By</th>
                                            <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-gray-500">Time</th>
                                            <th className="px-4 py-3 text-center text-xs font-bold uppercase tracking-wide text-gray-500">Players</th>
                                            <th className="px-4 py-3 text-center text-xs font-bold uppercase tracking-wide text-gray-500">Equipment</th>
                                        </tr>
                                    </thead>
                                    <tbody>
