                    ...
                    <Card className="border border-gray-900">
                        ...
                        <CardContent>
                            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                                <StatCard
                                    icon={<Swords className="w-5 h-5 text-blue-600" />}
                                ...
                                />
                                <StatCard
                                    icon={<Dumbbell className="w-5 h-5 text-[#004d40]" />}
                            ...
                            {tournaments === 0 && (
                                <p className="text-xs text-gray-400 mt-4 text-center">
                                    Tournament data is not yet tracked. Practice sessions are
                                ...
                                <div>
                                        ...
                                        {monthlyPractice.map(({ month, count }) => (
                                            ...
                                            >
                                                ...
                                                <div className="w-full flex items-end justify-center">
                                                    <div
                                                        className="w-full bg-[#004d40] rounded-t"
                                                        style={{
                                                            height: `${Math.round((count / maxMonthly) * 120)}px`,
                                                            minHeight: '4px',
                                                        }}
                                                    />
                                                </div>
                                    ...
                                    <div className="flex items-center gap-2 mt-3 justify-center">
                                        ...
                                        <span className="text-xs text-gray-500">
                                            Practice Sessions (completed bookings)
                                        </span>
                                    </div>
                                </div>
                            )}
    ...
    return (
        <div className={`rounded-lg p-4 ${bg} flex flex-col gap-2`}>
            <div className="flex items-center gap-2">
                {icon}
                <span
                    className={`text-xs font-medium ${highlight ? 'text-[#004d40]' : 'text-gray-600'}`}
                >
                    {label}
                </span>
            </div>
            <span
                className={`text-2xl font-bold ${highlight ? 'text-[#004d40]' : 'text-gray-900'}`}
            >
                {value}
            </span>
        </div>
